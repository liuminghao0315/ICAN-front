          ...
          <div class="archive-header">
            <div class="file-section">
              ...
              <div class="file-info">
                <!-- 文件名 + 时长 -->
                <div class="file-main">
                  <span class="file-name">{{ mockVideoArchive?.fileName || '视频' }}</span>
                ...
                <!-- 视频内容人物特征 -->
                <div v-if="analysisResult.videoInfo.mainCharacter && (analysisResult.videoInfo.mainCharacter.gender || analysisResult.videoInfo.mainCharacter.ageRange || analysisResult.videoInfo.mainCharacter.clothing || analysisResult.videoInfo.mainCharacter.voiceProfile)" class="content-features-row">
                  <div class="feature-label">
                    <el-icon :size="14"><User /></el-icon>
                    视频主要人物:
                  </div>
                  <span v-if="analysisResult.videoInfo.mainCharacter.gender" class="profile-tag">
                    <el-icon><Male v-if="analysisResult.videoInfo.mainCharacter.gender.includes('男')"/><Female v-else /></el-icon> {{ analysisResult.videoInfo.mainCharacter.gender }}
                  </span>
                  <span v-if="analysisResult.videoInfo.mainCharacter.ageRange" class="profile-tag">
                    <el-icon><Calendar /></el-icon> {{ analysisResult.videoInfo.mainCharacter.ageRange }}
                  </span>
                  <span v-if="analysisResult.videoInfo.mainCharacter.clothing" class="profile-tag">
                    <el-icon><School /></el-icon> {{ analysisResult.videoInfo.mainCharacter.clothing }}
                  </span>
                  <span v-if="analysisResult.videoInfo.mainCharacter.voiceProfile" class="profile-tag">
                    <el-icon><Microphone /></el-icon> {{ analysisResult.videoInfo.mainCharacter.voiceProfile }}
                  </span>
                </div>

                <!-- 检测到的关键内容 -->
                <div v-if="analysisResult.videoInfo.detectedKeywords && analysisResult.videoInfo.detectedKeywords.length > 0" class="detected-keywords-row">
                  <div class="feature-label">
                    <el-icon :size="14"><Search /></el-icon>
                    内容关键词:
                  </div>
                  <div class="keywords-container">
                    <span v-for="(kw, idx) in analysisResult.videoInfo.detectedKeywords" :key="idx" 
            ...
            <div class="global-stats-section stats-pro-container">
              ...
              <div class="stat-pro-item" :class="{ 'active': currentCardId === 'identity' }" @click="openEvidenceDrawer('identity')">
                ...
                <div class="pro-content">
                  <div class="pro-label">身份判定 <span class="evidence-badge">({{ analysisResult.identity.evidences?.length || 0 }})</span></div>
                  <div class="pro-value text-identity">
                    {{ mockIdentityAnalysis?.identityLabel || '-' }}
                  </div>
                  <div class="pro-subtitle">置信度 {{ mockIdentityAnalysis?.modalityFusion?.finalScore || 0 }}%</div>
                </div>
              </div>
              ...
              <div class="stat-pro-item" :class="{ 'active': currentCardId === 'university' }" @click="openEvidenceDrawer('university')">
                ...
                <div class="pro-content">
                  <div class="pro-label">涉及高校 <span class="evidence-badge">({{ analysisResult.university.evidences?.length || 0 }})</span></div>
                  <div class="pro-value text-uni">
                    {{ mockUniversityBaseline?.universityName || '-' }}
                  </div>
                  <div class="pro-subtitle">匹配度 {{ mockUniversityBaseline?.modalityFusion?.finalScore || 0 }}%</div>
                </div>
              </div>
              ...
              <div class="stat-pro-item" :class="{ 'active': currentCardId === 'topic' }" @click="openEvidenceDrawer('topic')">
                ...
                <div class="pro-content">
                  <div class="pro-label">内容主题 <span class="evidence-badge">({{ analysisResult.topic.evidences?.length || 0 }})</span></div>
              ...
              <div class="stat-pro-item" :class="{ 'active': currentCardId === 'attitude' }" @click="openEvidenceDrawer('attitude')">
                ...
                <div class="pro-content">
                  <div class="pro-label">对学校态度 <span class="evidence-badge">({{ analysisResult.attitude.evidences?.length || 0 }})</span></div>
                  ...
                  <div class="pro-subtitle">{{ mockContentAnalysis?.negativeMentionCount || 0 }}处负面，占比 {{ mockContentAnalysis ? Math.round((mockContentAnalysis.negativeMentionCount / mockContentAnalysis.schoolMentionCount) * 100) : 0 }}%</div>
                </div>
              </div>
              ...
              <div class="stat-pro-item" :class="{ 'active': currentCardId === 'opinionRisk' }" @click="openEvidenceDrawer('opinionRisk')">
                ...
                <div class="pro-icon" :class="getOpinionRiskIconClass(getRiskLevelByScore(analysisResult.opinionRisk.modalityFusion.finalScore))">
                  <el-icon><WarningFilled /></el-icon>
                </div>
                <div class="pro-content">
                  <div class="pro-label">
                    潜在舆论风险 <span class="evidence-badge">({{ analysisResult.opinionRisk.evidences?.length || 0 }})</span>
                    <span class="ai-predict-badge">AI预测</span>
                  </div>
                  <div class="pro-value" :class="getOpinionRiskTextClass(getRiskLevelByScore(analysisResult.opinionRisk.modalityFusion.finalScore))">
              ...
              <div class="stat-pro-item" :class="{ 'active': currentCardId === 'action' }" @click="openEvidenceDrawer('action')">
                ...
                <div class="pro-content">
                  <div class="pro-label">处置建议 <span class="evidence-badge">({{ analysisResult.action.evidences?.length || 0 }})</span></div>
        ...
        <div class="multi-modal-container">
          ...
          >
            <div class="video-player-wrapper">
              ...
              <!-- 实时风险状态指示器（左上角呼吸灯） -->
              <div v-show="realVideoUrl" class="risk-status-indicator" :class="getCurrentRiskClass()">
                ...
                <div class="timeline-progress-bar">
                  ...
                  <div 
                    ...
                    :class="[
                      ...
                      evidence.sentimentScore !== undefined ? `mark-sentiment-${getSentimentBySentimentScore(evidence.sentimentScore)}` : '',
                    ...
                    >
                      ...
                      <div class="evidence-content-inline">
                        <div class="evidence-desc-inline">
                          <!-- 情感标签（只对"对学校态度"卡片显示） -->
                          <span v-if="currentCardId === 'attitude' && evidence.sentimentScore !== undefined" 
                                class="sentiment-tag-inline" 
                                :class="'sentiment-' + getSentimentBySentimentScore(evidence.sentimentScore)">
                            {{ getSentimentText(getSentimentBySentimentScore(evidence.sentimentScore)) }}
                          </span>
              ...
              <div class="panel-header-compact">
                ...
                <div class="header-actions-group">
                  <div class="risk-filter-group">
                    <!-- 新增：模态筛选器 -->
                    <el-tooltip content="显示所有类型的事件" placement="bottom" :show-after="300">
                      <button 
                    ...
                    <el-tooltip content="只显示中高风险事件" placement="bottom" :show-after="300">
                      <button 
                    ...
                    <el-tooltip content="只显示语音台词" placement="bottom" :show-after="300">
                      <button 
              ...
              >
                ...
                >
                  ...
                  <div class="event-timeline-anchor">
                    ...
                    <div class="event-dot" :class="[`risk-${getRiskLevel(event.riskScore)}`, `modality-${event.modality}`]">
                      <el-icon :size="12">
                    ...
                    <template v-if="event.modality === 'speech'">
                      ...
                      <transition name="expand">
                        <div v-if="currentPlayTime >= event.startTime && currentPlayTime <= event.endTime" class="speech-meta">
                          ...
                          <span v-if="getRiskLevel(event.riskScore) !== 'low'" class="meta-tag risk" :class="`risk-${getRiskLevel(event.riskScore)}`">
                            {{ getRiskLevel(event.riskScore) === 'high' ? '高风险' : '中风险' }}
                          </span>
                          <el-tooltip 
                      ...
                      <div class="system-notification" :class="`notif-${event.modality}`">
                        ...
                        <transition name="expand">
                          <div v-if="currentPlayTime >= event.startTime && currentPlayTime <= event.endTime" class="notif-detail">
                            ...
                            <span v-if="getRiskLevel(event.riskScore) !== 'low'" class="detail-chip risk" :class="`risk-${getRiskLevel(event.riskScore)}`">
                              {{ getRiskLevel(event.riskScore) === 'high' ? '高风险' : '中风险' }}
                            </span>
                          </div>
                        </transition>
        ...
        <div class="side-metrics-wrapper" v-if="false">
          <div class="key-metrics-grid">
            ...
            <div class="neu-card metric-mini">
              ...
              <div class="metric-content">
                <div class="metric-value">{{ analysisResult.isUniversityRelated ? '是' : '否' }}</div>
...
// 导入Element Plus图标
import { 
  User, School, ChatDotRound, TrendCharts, WarningFilled, DocumentChecked, 
  Male, Female, Menu, View, Headset, VideoPause, InfoFilled, Download,
  Upload, VideoCamera, Clock, Calendar, Search, Microphone, VideoPlay
...
const mockIdentityAnalysis = computed(() => currentResult.value?.identity || null)
const mockUniversityBaseline = computed(() => currentResult.value?.university || null)

// 计算attitude的统计数据（前端根据sentimentScore统计）
const attitudeStatistics = computed(() => {
  if (!currentResult.value) return { positive: 0, neutral: 0, negative: 0, total: 0 }
...
const mockOpinionRisk = computed(() => {
  if (!currentResult.value) return null
  const riskLevel = getRiskLevelByScore(currentResult.value.opinionRisk.modalityFusion.finalScore)
  return {
...
// 提取时间轴数据（响应式）
const timeGranularity = computed(() => currentResult.value?.timelineData.timeGranularity || 5)
const mockVideoRisksData = computed(() => currentResult.value?.timelineData.videoRisks || [])
const mockAudioEmotionsData = computed(() => currentResult.value?.timelineData.audioEmotions || [])
const mockTextRisksData = computed(() => currentResult.value?.timelineData.textRisks || [])
const mockComprehensiveRisksData = computed(() => currentResult.value?.timelineData.comprehensiveRisks || [])
const mockRadarDataByTime = computed(() => currentResult.value?.timelineData.radarByTime || [])
const mockAverageRadarData = computed(() => currentResult.value?.timelineData.averageRadarData || [0, 0, 0, 0, 0, 0])
// 提取辅助分析数据
...
const cardsData = computed<CardData[]>(() => {
  ...
  return [
    ...
    },
    {
      ...
      label: '潜在舆论风险',
      get value() {
        const riskLevel = getRiskLevelByScore(result.opinionRisk.modalityFusion.finalScore)
...
const currentRadarData = computed(() => {
  ...
  const index = Math.min(Math.floor(currentTime / timeGranularity.value), radarData.length - 1)
...
const filteredTimelineEvents = computed(() => {
  ...
  if (modalityFilter.value === 'speech-only') {
    ...
    events = events.filter(e => getRiskLevel(e.riskScore) === 'high' || getRiskLevel(e.riskScore) === 'medium')
  }
  // 'all' 模式：显示所有模态
...
const getSentimentBySentimentScore = (sentimentScore: number | undefined): string => {
...
const highRiskSegmentCount = computed(() => {
  return timelineEvents.value.filter(e => getRiskLevel(e.riskScore) === 'high').length
})
  ...
  return {
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        const index = Math.min(Math.floor(currentTime / timeGranularity.value), comprehensiveRisks.length - 1)
        const comprehensiveRisk = comprehensiveRisks[index]
        const avgRisk = comprehensiveRisk?.intensity ? comprehensiveRisk.intensity * 100 : 0  // 转为百分比
    ...
    series: [
      {
        ...
        data: [
          { 
            value: Math.round((result.opinionRisk.modalityFusion?.finalScore || 0)), 
            name: '风险评分', 
            itemStyle: { color: '#f56c6c' } 
          },
          { 
            value: Math.round(100 - (result.opinionRisk.modalityFusion?.finalScore || 0)), 
            name: '安全评分', 
...
const initializeComponent = () => {
  ...
  // 3. 初始化默认选中第一个高风险事件
  nextTick(() => {
    const firstHighRisk = timelineEvents.value.find(e => getRiskLevel(e.riskScore) === 'high')
    if (firstHighRisk) {
...
const onVideoLoaded = () => {
  ...
  if (selectedEvidenceId.value) {
    const event = timelineEvents.value.find(e => e.id === selectedEvidenceId.value)
...
const scrollToActiveEvents = () => {
  ...
  const activeElements = container.querySelectorAll('.timeline-event-item.is-active')
  ...
  requestAnimationFrame(() => {
    ...
    if (activeElements.length === 1) {
      ...
      const activeCenter = activeRect.top - containerRect.top + container.scrollTop + activeRect.height / 2
      ...
      const boundingBoxTop = firstRect.top - containerRect.top + container.scrollTop
      const boundingBoxBottom = lastRect.bottom - containerRect.top + container.scrollTop
...
const updateTextEvidenceTooltipPosition = () => {
  nextTick(() => {
    ...
    items.forEach((item, index) => {
      ...
      if (rowItems.length > 0 && Math.abs(rect.top - rowItems[0].getBoundingClientRect().top) > 5) {
...
.interactive-view {
  ...
  @keyframes breathing-green {
    ...
    50% {
      box-shadow: 0 0 12px rgba(16, 185, 129, 0.9), 0 0 20px rgba(16, 185, 129, 0.4);
      opacity: 0.7;
    }
  }
